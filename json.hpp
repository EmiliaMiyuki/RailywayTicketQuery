...
namespace Json {
	...
	public:
		...
		void _ass_p() { if (isPointerValues() && hasValue()) delete val.value_string; }
	...
	void deleteObj(Object * o) {
		static int di = 0;
		if (!o || !o->val.value_string) //¼ì²éval_stringµØÖ·ÊÇ·ñÎªnullptrÆäÊµ¾ÍµÈÓÚ¼ì²éÕû¸övalÊÇ²»ÊÇÎª0£¬Èç¹ûÎª0¾ÍÖ±½Ó·µ»Ø±ÜÃâÏÂÃæÈ¥±éÀú
			return;
	...
	*/
	...
	class parser {
		...
		void    E(Array * inh);
		template <class T>
		void match(T terminal, Exception * e = nullptr, bool releaseMemory = false);
	public:
	...
	template <class T>
	void parser::match(T terminal, Exception * e, bool releaseMemory) {
		if (*lookahead == terminal) {
			if (releaseMemory || lookahead->type == Object::SIGN)
				delete lookahead;
			lookahead = lex->scan();
		}
		else {
